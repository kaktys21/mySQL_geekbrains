...
-- Schema vk
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `vk` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci ;
USE `vk` ;
...
CREATE TABLE IF NOT EXISTS `vk`.`friend_request` (
  ...
  INDEX `fk_friend_request_user2_idx` (`to_user_id` ASC) INVISIBLE,
...
CREATE TABLE IF NOT EXISTS `vk`.`media` (
  ...
  `user_id` INT UNSIGNED NOT NULL,
  `url` VARCHAR(45) NULL DEFAULT NULL COMMENT '\\files\\2020\\01\\file.jpg',
...
CREATE TABLE IF NOT EXISTS `vk`.`likes` (
  ...
  `media_type_id` INT UNSIGNED NULL,
  `from_user_id` INT UNSIGNED NULL,
  INDEX `fk_likes_user1_idx` (`user_id` ASC) VISIBLE,
  INDEX `fk_likes_post1_idx` (`post_id` ASC) VISIBLE,
  INDEX `fk_likes_media_type1_idx` (`media_type_id` ASC) VISIBLE,
  INDEX `fk_likes_user2_idx` (`from_user_id` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_likes_post1`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_likes_media_type1`
    FOREIGN KEY (`media_type_id`)
    REFERENCES `vk`.`media_type` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
/*
Правильно понимаю, что вот этот ключ нужно сделать, 
чтоб ссылаться на реального пользователя в лайках?
*/
  CONSTRAINT `fk_likes_user2`
    FOREIGN KEY (`from_user_id`)
    REFERENCES `vk`.`user` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
